...
contract DSCEngineTest is Test {
    //     event CollateralRedeemed(address indexed redeemFrom, address indexed redeemTo, address token, uint256
    // amount); // if
    ...
    function testCanDepositedCollateralAndGetAccountInfo() public depositedCollateral {
        ...
        assertEq(totalDscMinted, expectedDepositAmount);
    ...
    //     uint256 amountToMint = (amountCollateral * (uint256(price) * dscEngine.getAdditionalFeedPrecision())) /
    ...
    //         // 180*50 (LIQUIDATION_THRESHOLD) / 100 (LIQUIDATION_PRECISION) / 100 (PRECISION) = 90 / 100
    // (totalDscMinted) =
    //         // 0.9
    ...
    //         uint256 expectedUserCollateralValueInUsd = dsce.getUsdValue(weth, amountCollateral) -
    // (usdAmountLiquidated);
